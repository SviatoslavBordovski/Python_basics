...
def test_setup(): #setup and clean up the tests
    ...
    yield #using a 'yield' statement instead of a 'return' statement, all code would run after a 'yield' statement as a teardown
    driver.close()
    driver.quit()
...
@pytest.mark.skip(reason='test is not in scope of this sprint') #test would be skipped, reason could be specified in the brackets
